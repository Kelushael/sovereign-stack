...
# Ollama model names (pulled via `ollama pull`)
OLLAMA_MODELS = {
...
class ModelManager:
    def __init__(self):
        ...
        self.mode = 'ollama'  # 'ollama' or 'gguf'

    def resolve_ollama(self, name):
        if not name or name in ('current', 'default'):
            return self.current
        n = name.lower().strip()
        if n in OLLAMA_MODELS:
            return OLLAMA_MODELS[n]
        return n  # pass through raw name
    ...
    def available(self):
        ollama = self.available_ollama()
        gguf = self.available_gguf()
        return ollama + gguf
...
def run_inference(model_name, messages, max_tokens=2048, temperature=0.7):
    ...
    # Try ollama first (this box is ollama-native)
    try:
        data = json.dumps({
            'model': model_name,
            'prompt': prompt,
...
class AmalloHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path in ('/v1/chat/completions', '/api/generate', '/api/chat'):
            ...
            text = run_inference(model_name, messages,
